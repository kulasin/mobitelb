			...
			success : function (data) { 
				if (data !== '') {
					...
					if (data.slice(0, 6) === 'finish') {
						newHTML = jQuery(data.replace(/cmsmasters_post_timeline(?!_)/g, 'cmsmasters_post_timeline shortcode_animated').replace(/cmsmasters_post_masonry/g, 'cmsmasters_post_masonry shortcode_animated').slice(6, -1));
						
						
						blogContainer.find('.cmsmasters_wrap_more_items').hide();
					} else {
						newHTML = jQuery(data.replace(/cmsmasters_post_timeline(?!_)/g, 'cmsmasters_post_timeline shortcode_animated').replace(/cmsmasters_post_masonry/g, 'cmsmasters_post_masonry shortcode_animated'));
